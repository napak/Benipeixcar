...
function hookProcessInput(HookEvent $event) {
  $form = $event->object; 
  $field = $form->get('username'); // substitute the name of your username field
  if(!$field) return; // probably a different form you don't want
  $username = wire('sanitizer')->pageName($field->attr('value'));
  $user = wire('users')->get($username);
  if($user->id) $field->error("Ya existe un usuario con ese nombre de usuario"); 
}
...
if($form) {
	...
	foreach(array('jquery-ui', 'inputfields', 'main') as $file) {
		$path = $config->paths->FormBuilder . "themes/$form->theme/$file";
		$url = $config->urls->FormBuilder . "themes/$form->theme/$file";
